...
public class BookCaseSaveController implements Controller {
	...
	//사용자가 업로드한 파일이 저장될 서버 경로(파일세스템 경로)
	...
	//업로드 가능한 최대 크기(바이트)
	int maxSize = 10*1024*1024;		//1024 바이트=1KB , 10MByte 로 설정
	
	//request 는 파일을 받을 수 있는 reqeust로 사용해야 합니다. : cos 라이브러리 추가
	...
	//기존 방식처럼 text 타입 등 파라미터 받아오기 - 문자열 파라미터도 MultipartReqeust 를 사용합니다.
